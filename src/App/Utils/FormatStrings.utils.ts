export const capitalize = (string: string) => {
  ...
  const stringCapitalize =
    stringWithUnderLine[0].toUpperCase() + stringWithUnderLine.slice(1);
...
export const slugToText = (string: string) => {
  const normalizeText = string.replace(/-/gi, " ");
  const capitalizeText = capitalizeStrings(normalizeText);
